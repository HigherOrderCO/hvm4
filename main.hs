...
-- main = test
-- 
-- - PROBLEM: the language implemented on main.hs is overly complex.
--
-- refactor it so that the term type becomes:
--
-- data Term
--   = Var !Name
--   | Cop !Int !Name
...
--   deriving (Eq)
--
-- notice that we removed all type formers like Set, Emp, Uni, All, all data
-- constructors like Zer, Suc, Nil, Con, and all eliminators, like Swi, Mat.
-- instead, we include just a single unified constructor type, named Ctr:
--
-- | Ctr ::= "#" Name "{" [Term] "}"
--
-- and a single pattern-matching form, named Mat:
--
-- | Mat ::= "λ" "{" "#" Name ":" Term ";" Term "}"
--
-- where the first Term is the body of that matched case, and the second term is
-- the default case. when pattern-matching, we do it like so:
--
-- case x of
--   ...
--   Ctr k z   ->
--   Mat k h m ->
--
-- i.e., we use 'k' for "constructor name", 'z' for "constructor fields", 'h' for
-- "matched case" (hit), and 'm' for "default case" (miss)
--
...
-- (m #B{x,y,z,...})
--
-- this will *greatly* simplify the file above.
--
-- your goal is to rewrite the file from scratch using this new paradigm.
-- keep the exact same structure, just delete all removed ctors, and add Ctr/Mat instead
--
-- adjust ALL parts of the file as if the spec was written like that to begin with
-- 
-- port ALL the old tests to the new language (VERY IMPORTANT)
-- make sure they pass
-- 
-- also, for simplicity, let's just use Strings for Names, removing completely Int Names.
...
data Env = Env
  { env_book    :: !Book
  , env_itrs    :: !(IORef Int)
  , env_fresh   :: !(IORef String)
  , env_sub_map :: !(IORef (M.Map Name Term))
  , env_dup_map :: !(IORef (M.Map Name (Lab, Term)))
  }

-- now, implement the Show session, and nothing else
...
instance Show Term where
  show (Var k)       = k
  show (Cop s k)     = k ++ (if s == 0 then "₀" else "₁")
  show (Ref k)       = "@" ++ k
  show (Nam k)       = k
  show (Dry f x)     = show_app f [x]
  show Era           = "&{}"
  show (Sup l a b)   = "&" ++ show l ++ "{" ++ show a ++ "," ++ show b ++ "}"
  show (Dup k l v t) = "!" ++ k ++ "&" ++ show l ++ "=" ++ show v ++ ";" ++ show t
  show (Lam k f)     = "λ" ++ k ++ "." ++ show f
  show (App f x)     = show_app f [x]
  show (Ctr k xs)    = "#" ++ k ++ "{" ++ intercalate "," (map show xs) ++ "}"
  show (Mat k h m)   = "λ{#" ++ k ++ ":" ++ show h ++ ";" ++ show m ++ "}"
  show (And a b)     = show a ++ "&&" ++ show b
  show (Eql a b)     = show a ++ "==" ++ show b
  show (Gua f g)     = show f ++ "~>" ++ show g
...
instance Show Book where
  show (Book m) = unlines [ "@" ++ k ++ " = " ++ show ct | (k, ct) <- M.toList m ]

show_dup_map :: M.Map Name (Lab, Term) -> String
show_dup_map m = unlines [ "! " ++ k ++ " &" ++ show l ++ " = " ++ show v | (k, (l, v)) <- M.toList m ]

show_sub_map :: M.Map Name Term -> String
show_sub_map m = unlines [ k ++ " ← " ++ show v | (k, v) <- M.toList m ]

-- now, implement all name utils
...
inc :: Name -> Name
inc []       = [alphabet !! 0]
inc (c : cs) = case elemIndex c alphabet of
  Just 63 -> alphabet !! 0 : inc cs
  Just n  -> alphabet !! (n + 1) : cs
  Nothing -> error $ "Invalid name char: " ++ [c]

fresh :: Env -> IO Name
fresh e = do
  name <- readIORef (env_fresh e)
  let new_name = inc name
  writeIORef (env_fresh e) new_name
  return new_name
...
-- We need a way to convert label names to Ints if Lab is Int.
name_to_int :: String -> Int
name_to_int = foldl' (\acc c -> (acc `shiftL` 6) + idx c) 0
  where idx c = maybe 0 id (elemIndex c alphabet)
...
parse_label :: ReadP Int
parse_label = choice
  [ parse_number
  , name_to_int <$> parse_name
  ]
...
parse_ctr = do
  ...
  k <- parse_name
  lexeme (char '{')
  xs <- sepBy parse_term (lexeme (char ','))
  lexeme (char '}')
  return (Ctr k xs)
...
parse_var = do
  k <- parse_name
  choice
    [ string "₀" >> return (Cop 0 k)
    , string "₁" >> return (Cop 1 k)
    , return (Var k)
    ]
...
new_env bk = do
  itr <- newIORef 0
  frs <- newIORef "a"
  sub <- newIORef M.empty
  dm  <- newIORef M.empty
  return $ Env bk itr frs sub dm

inc_itrs :: Env -> IO ()
inc_itrs e = modifyIORef' (env_itrs e) (+ 1)

take_dup :: Env -> Name -> IO (Maybe (Lab, Term))
take_dup e k = atomicModifyIORef' (env_dup_map e) $ \m -> (M.delete k m, M.lookup k m)

take_sub :: Env -> Name -> IO (Maybe Term)
take_sub e k = atomicModifyIORef' (env_sub_map e) $ \m -> (M.delete k m, M.lookup k m)

make_dup :: Env -> Name -> Lab -> Term -> IO ()
make_dup e k l v = modifyIORef' (env_dup_map e) (M.insert k (l, v))

subst :: Env -> Name -> Term -> IO ()
subst e k v = modifyIORef' (env_sub_map e) (M.insert k v)
      ...
      case f of
        ...
        Mat k h m -> do
          ...
          case x of
            Era        -> wnf_app_mat_era e f x
            Sup{}      -> wnf_app_mat_sup e f x
            Ctr c args -> wnf_app_mat_ctr e k h m c args
            _          -> return (App f x)
        Nam{}     -> wnf_app_nam e f x
        Dry{}     -> wnf_app_dry e f x
        Gua f g  -> do
          ...
          case g of
            ...
            Sup{}   -> wnf_app_gua_sup e f g x
            Lam{}   -> wnf_app_gua_lam e f g x
            Mat{}   -> wnf_app_gua_mat e f g x
            Ctr{}   -> return (App (Gua f g) x)
            Gua{}   -> wnf_app_gua_gua e f g x
            _       -> return (App (Gua f g) x)
        _       -> return (App f x)
    And a b -> do
      ...
      case a of
        Era        -> wnf_and_era e a b
        Sup{}      -> wnf_and_sup e a b
        Ctr "F" [] -> wnf_and_fal e a b
        Ctr "T" [] -> wnf_and_tru e a b
        _          -> return (And a b)
    Eql a b -> do
      a <- wnf e a
      case a of
...
wnf_dup_ctr :: WnfDup
wnf_dup_ctr i e k l (Ctr c args) = do
  inc_itrs e
  args_p <- mapM (clone e l) args
  let args0 = map fst args_p
  let args1 = map snd args_p
  if i == 0 then do
    subst e k (Ctr c args1)
    wnf e (Ctr c args0)
  else do
    subst e k (Ctr c args0)
    wnf e (Ctr c args1)

wnf_dup_mat :: WnfDup
wnf_dup_mat i e k l (Mat n h m) = do
  inc_itrs e
  (h0, h1) <- clone e l h
  (m0, m1) <- clone e l m
  if i == 0 then do
    subst e k (Mat n h1 m1)
    wnf e (Mat n h0 m0)
  else do
    subst e k (Mat n h0 m0)
    wnf e (Mat n h1 m1)
...
wnf_app_mat_sup :: Env -> Term -> Term -> IO Term
wnf_app_mat_sup e (Mat n h m) (Sup l x y) = do
  inc_itrs e
  (n0, n1) <- clone e l (Mat n h m)
  wnf e (Sup l (App n0 x) (App n1 y))

wnf_app_mat_ctr :: Env -> Name -> Term -> Term -> Name -> [Term] -> IO Term
wnf_app_mat_ctr e mk h m ck args = do
  inc_itrs e
  if mk == ck then
    wnf e (foldl' App h args)
  else
    wnf e (App m (Ctr ck args))
...
wnf_app_gua_mat :: Env -> Term -> Term -> Term -> IO Term
wnf_app_gua_mat e f g@(Mat k h m) x = do
  inc_itrs e
  wnf e (Gua (App f x) (App g x))
...
wnf_and_fal :: Env -> Term -> Term -> IO Term
wnf_and_fal e _ b = do
  inc_itrs e
  wnf e (Ctr "F" [])

wnf_and_tru :: Env -> Term -> Term -> IO Term
wnf_and_tru e _ b = do
  inc_itrs e
  wnf e b
...
wnf_eql_ctr_ctr e (Ctr ak args1) (Ctr bk args2) = do
  inc_itrs e
  if ak == bk && length args1 == length args2 then
    wnf e (foldr And (Ctr "T" []) (zipWith Eql args1 args2))
  else
    wnf e (Ctr "F" [])

wnf_eql_mat_mat :: WnfEql
wnf_eql_mat_mat e (Mat ak ah am) (Mat bk bh bm) = do
  inc_itrs e
  if ak == bk then
    wnf e (And (Eql ah bh) (Eql am bm))
  else
    wnf e (Ctr "F" [])
...
wnf_ref e k = do
  ...
  case M.lookup k m of
    Just f  -> do
      ...
      g <- alloc e f
      wnf e g
    Nothing -> error $ "UndefinedReference: " ++ k

-- Allocation
-- ==========

alloc :: Env -> Term -> IO Term
alloc e term = go M.empty term where

  go :: M.Map Name Name -> Term -> IO Term

  go m (Var k) = do
    return $ Var (M.findWithDefault k k m)

  go m (Cop s k) = do
    return $ Cop s (M.findWithDefault k k m)
  ...
  go m (Dup k l v t) = do
    k' <- fresh e
    v' <- go m v
    t' <- go (M.insert k k' m) t
    return $ Dup k' l v' t'

  go m (Lam k f) = do
    k' <- fresh e
    f' <- go (M.insert k k' m) f
    return $ Lam k' f'

-- Normalization
-- =============

snf :: Env -> Term -> IO Term
snf e x = do
  !x' <- wnf e x
  case x' of
...
book = unlines
  ...
  , "@K8  = λs. !S0&K=s; !S1&K=λx0.(S0₀ (S0₁ x0)); !S2&K=λx1.(S1₀ (S1₁ x1)); λx3.(S2₀ (S2₁ x3))"
  , "@id  = ^id ~> λa.a"
  , "@not = ^not ~> λ{#Z:#S{#Z{}};λ{#S:λp.#Z{};#Z{}}}"
  , "@dbl = ^dbl ~> λ{#Z:#Z{};λ{#S:λp.#S{#S{(@dbl p)}};#Z{}}}"
  , "@and = ^and ~> λ{#Z:λ{#Z:#Z{};λ{#S:λp.#Z{};&{}};λ{#S:λp.λ{#Z:#Z{};λ{#S:λp.#S{#Z{}};#Z{}};#Z{}}}"
--  , "@add = ^add ~> λ{#Z:λb.b;λ{#S:λa.λb.#S{(@add a b)};#Z{}}}"
--   , "@sum = ^sum ~> λ{#Z:#Z{};λ{#S:λp.!P&S=p;#S{(@add P₀ (@sum P₁))};#Z{}}}"
--   , "@foo = ^foo ~> &L{λx.x,λ{#Z:#Z{};λ{#S:λp.p;#Z{}}}"
--   , "@gen = ^gen ~> !F&A=@gen &A{λx.!X&B=x;&B{X₀,#S{X₁}},λ{#Z:&C{#Z{},#S{#Z{}}};λ{#S:λp.!G&D=F₁;!P&D=p;&D{(G₀ P₀),!H&E=G₁;!Q&E=P₁;#S{&E{(H₀ Q₀),#S{(H₁ Q₁)}}}}};#Z{}}}"
--   , "@prd = ^prd ~> λ{#Z:#Z{};λ{#S:λp.p;#Z{}}}"
  ]
...
--   , ("!F&L=@id;!G&L=F₀;λx.(G₁ x)", "λa.a")
--   , ("(@and #Z{} #Z{})", "#Z{}")
--   , ("(@and &L{#Z{},#S{#Z{}}} #S{#Z{}})", "&L{#Z{},#S{#Z{}}}")
--   , ("(@and &L{#S{#Z{}},#Z{}} #S{#Z{}})", "&L{#S{#Z{}},#Z{}}")
--   , ("(@and #S{#Z{}} &L{#Z{},#S{#Z{}}})", "&L{#Z{},#S{#Z{}}}")
--   , ("(@and #S{#Z{}} &L{#S{#Z{}},#Z{}})", "&L{#S{#Z{}},#Z{}}")
--   , ("λx.(@and #Z{} x)", "λa.(and #Z{} a)")
--   , ("λx.(@and x #Z{})", "λa.(and a #Z{})")
...
--   , ("(λf.!F00&A=f;!F01&A=λx00.(F00₀ (F00₁ x00));λx01.(F01₀ (F01₁ x01)) λX.(X λT0.λF0.F0 λT1.λF1.T1) λT2.λF2.T2)", "λa.λb.a")
--   , ("#S{&L{#Z{},#S{#Z{}}}}", "&L{#S{#Z{}},#S{#S{#Z{}}}}}")
--   , ("#S{&A{&B{#Z{},#S{#Z{}}},&C{#S{#S{#Z{}}},#S{#S{#S{#Z{}}}}}}}", "&A{&B{#S{#Z{}},#S{#S{#Z{}}}},&C{#S{#S{#S{#Z{}}}},#S{#S{#S{#S{#Z{}}}}}}}")
--   , ("λa.!A&L=a;&L{A₀,A₁}", "&L{λa.a,λa.a}")
--   , ("λa.λb.!A&L=a;!B&L=b;&L{λx.(x A₀ B₀),λx.(x A₁ B₁)}", "&L{λa.λb.λc.(c a b),λa.λb.λc.(c a b)}")
--   , ("λt.(t &A{#S{#Z{}},#S{#S{#Z{}}}} #S{#S{#S{#Z{}}}})", "&A{λa.(a #S{#Z{}} #S{#S{#S{#Z{}}}}),λa.(a #S{#S{#Z{}}} #S{#S{#S{#Z{}}}})}") 
--   , ("λt.(t #S{#Z{}} &B{#S{#S{#S{#Z{}}}},#S{#S{#S{#S{#Z{}}}}}})", "&B{λa.(a #S{#Z{}} #S{#S{#S{#Z{}}}}),λa.(a #S{#Z{}} #S{#S{#S{#S{#Z{}}}}})}")
--   , ("λt.(t &A{#S{#Z{}},#S{#S{#Z{}}}} &A{#S{#S{#S{#Z{}}}},#S{#S{#S{#S{#Z{}}}}}})", "&A{λa.(a #S{#Z{}} #S{#S{#S{#Z{}}}}),λa.(a #S{#S{#Z{}}} #S{#S{#S{#S{#Z{}}}}})}")
--   , ("λt.(t &A{#S{#Z{}},#S{#S{#Z{}}}} &B{#S{#S{#S{#Z{}}}},#S{#S{#S{#S{#Z{}}}}}})", "&A{&B{λa.(a #S{#Z{}} #S{#S{#S{#Z{}}}}),λa.(a #S{#Z{}} #S{#S{#S{#S{#Z{}}}}})},&B{λa.(a #S{#S{#Z{}}} #S{#S{#S{#Z{}}}}),λa.(a #S{#S{#Z{}}} #S{#S{#S{#S{#Z{}}}}})}}") 
--   , ("@gen", "&A{&B{λa.a,λa.#S{a}},&C{&D{λ{#Z:#Z{};λ{#S:λa.(gen a);#Z{}};#Z{}},&E{λ{#Z:#Z{};λ{#S:λa.#S{(gen a)};#Z{}},λ{#Z:#Z{};λ{#S:λa.#S{#S{(gen a)}};#Z{}}}},&D{λ{#Z:#S{#Z{}};λ{#S:λa.(gen a);#Z{}};#Z{}},&E{λ{#Z:#S{#Z{}};λ{#S:λa.#S{(gen a)};#Z{}},λ{#Z:#S{#Z{}};λ{#S:λa.#S{#S{(gen a)}};#Z{}}}}}}") 
--   , ("λx.(@gen #S{#S{x}})", "&A{&B{λa.#S{#S{a}},λa.#S{#S{#S{a}}}},&D{λa.(gen a),&E{λa.#S{#S{(gen a)}},λa.#S{#S{#S{#S{(gen a)}}}}}}}")
--   , ("(@gen #S{#S{#Z{}}})", "&A{&B{#S{#S{#Z{}}},#S{#S{#S{#Z{}}}}},&D{&C{#Z{},#S{#Z{}}},&E{&C{#S{#S{#Z{}}},#S{#S{#S{#Z{}}}}},&C{#S{#S{#S{#S{#Z{}}}}},#S{#S{#S{#S{#S{#Z{}}}}}}}}}}") 
--   , ("!f&DUP=@prd; (f₀ (f₁ #Z{}))", "#Z{}")
--   , ("#S{#S{#Z{}}} == #S{#S{#Z{}}}", "#T{}")
--   , ("#S{#S{#S{#Z{}}}} == #S{#S{#Z{}}}", "#F{}")
--   , ("(λa.λb.a) == (λx.λy.x)", "#T{}")
